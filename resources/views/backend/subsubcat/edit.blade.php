...
@section('main')
    ...
    <main id="main-container">
        ...
        <div class="bg-body-light">
            <div class="content content-full">
                <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                    <h1 class="flex-grow-1 fs-3 fw-semibold my-2 my-sm-3">Edit Sub-Sub Category</h1>
                    <nav class="flex-shrink-0 my-2 my-sm-0 ms-sm-3" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Edit Sub-Sub Category</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="content">
            ...
            <form action="{{ route('update.subsubcat', $editData->id) }}" method="POST">
                @csrf
                <!-- Regular -->
                <div class="content-header col-11 mb-4">
                    <h2 class="content-heading">Edit Sub-Sub Category</h2>
                    ...
                    <div class="col-lg-10 col-xl-10" style="margin-left: 40px">
                        <div class="row mb-4">
                            <div class="col-6 mb-4">
                                ...
                                <input type="text" class="form-control" id="subsub_title" name="subsub_title" value="{{ $editData->subsub_title }}" placeholder="Enter sub-sub title" required>
                            </div>
                            ...
                            <div class="col-6">
                                ...
                                <select class="js-select2 form-select js-select2-enabled select2-hidden-accessible" id="category_id" name="category_id" style="width: 100%;" data-placeholder="Choose one.." data-select2-id="dm-ecom-product-category" tabindex="-1" aria-hidden="true">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ $category->id == $editData->category_id ? 'selected':'' }} >{{ $category->category_title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-6">
                                ...
                                <select class="js-select2 form-select js-select2-enabled select2-hidden-accessible" id="subcategory_id" name="subcategory_id" style="width: 100%;" data-placeholder="Choose one.." data-select2-id="dm-ecom-product-category" tabindex="-1" aria-hidden="true">
                                    ...
                                    @foreach($subcategories as $subcat)
                                        <option value="{{ $subcat->id }}" {{ $subcat->id == $editData->subcategory_id ? 'selected':'' }} >{{ $subcat->subcategory_title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
